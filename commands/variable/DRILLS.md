...
> **Namespacing:** create only objects prefixed `zz-variable-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh variable list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-variable-*` object created.
