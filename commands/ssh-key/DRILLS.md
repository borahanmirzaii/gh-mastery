...
> **Namespacing:** create only objects prefixed `zz-ssh-key-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh ssh-key list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-ssh-key-*` object created.
