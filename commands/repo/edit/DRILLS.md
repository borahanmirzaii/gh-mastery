...
> **Namespacing:** create only objects prefixed `zz-repo edit-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh repo edit list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-repo edit-*` object created.
