...
> **Namespacing:** create only objects prefixed `zz-repo-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh repo list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-repo-*` object created.
