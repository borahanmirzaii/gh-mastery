...
> **Namespacing:** create only objects prefixed `zz-status-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh status list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-status-*` object created.
