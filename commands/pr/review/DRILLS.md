...
> **Namespacing:** create only objects prefixed `zz-pr review-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh pr review list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-pr review-*` object created.
