...
> **Namespacing:** create only objects prefixed `zz-pr create-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh pr create list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-pr create-*` object created.
