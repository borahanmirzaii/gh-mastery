...
> **Namespacing:** create only objects prefixed `zz-attestation-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh attestation list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-attestation-*` object created.
