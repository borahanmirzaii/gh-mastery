...
> **Namespacing:** create only objects prefixed `zz-codespace-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh codespace list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-codespace-*` object created.
