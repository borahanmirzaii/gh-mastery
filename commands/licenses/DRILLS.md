...
> **Namespacing:** create only objects prefixed `zz-licenses-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh licenses list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-licenses-*` object created.
