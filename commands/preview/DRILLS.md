...
> **Namespacing:** create only objects prefixed `zz-preview-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh preview list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-preview-*` object created.
