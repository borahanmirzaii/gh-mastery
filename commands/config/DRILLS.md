...
> **Namespacing:** create only objects prefixed `zz-config-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh config list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-config-*` object created.
