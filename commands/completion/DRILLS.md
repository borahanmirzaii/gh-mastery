...
> **Namespacing:** create only objects prefixed `zz-completion-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh completion list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-completion-*` object created.
