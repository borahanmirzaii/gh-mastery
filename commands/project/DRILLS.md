...
> **Namespacing:** create only objects prefixed `zz-project-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh project list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-project-*` object created.
