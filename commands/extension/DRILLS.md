...
> **Namespacing:** create only objects prefixed `zz-extension-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh extension list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-extension-*` object created.
