...
> **Namespacing:** create only objects prefixed `zz-search-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh search list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-search-*` object created.
