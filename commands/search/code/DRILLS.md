...
> **Namespacing:** create only objects prefixed `zz-search code-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh search code list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-search code-*` object created.
