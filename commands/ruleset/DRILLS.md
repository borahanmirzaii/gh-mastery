...
> **Namespacing:** create only objects prefixed `zz-ruleset-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh ruleset list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-ruleset-*` object created.
