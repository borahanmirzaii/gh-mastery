...
> **Namespacing:** create only objects prefixed `zz-auth-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh auth list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-auth-*` object created.
