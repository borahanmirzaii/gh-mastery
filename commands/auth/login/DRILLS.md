...
> **Namespacing:** create only objects prefixed `zz-auth login-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh auth login list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-auth login-*` object created.
