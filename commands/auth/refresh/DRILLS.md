...
> **Namespacing:** create only objects prefixed `zz-auth refresh-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh auth refresh list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-auth refresh-*` object created.
