...
> **Namespacing:** create only objects prefixed `zz-issue develop-*` and delete them at the end, so parallel drills never collide.
...
```bash
gh issue develop <sub> ...
```
</details>

**Verify:** `gh issue develop list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-issue develop-*` object created.
