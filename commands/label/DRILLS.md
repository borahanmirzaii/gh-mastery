...
> **Namespacing:** create only objects prefixed `zz-label-*` and delete them at the end, so parallel drills never collide.
...
**Verify:** `gh label list ...` → expect <observable outcome>.
...
**Verify:** <end state you can check>. **Cleanup:** delete every `zz-label-*` object created.
